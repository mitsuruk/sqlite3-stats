...
本リファレンスでは **全258関数** を記載している。

`stat_bonferroni` はウィンドウ関数形式とスカラー形式の両方を持つため、
ウィンドウ関数とスカラー関数 — 検定補助の両カテゴリに記載している。
このためカテゴリ別の合計は 258 に対して 259 件になる。
...
| [スカラー関数 — 検定補助](ref/scalar_tests_helpers-ja.md) | 38 | DB非依存: 分布関数、特殊関数、比率検定、多重検定補正等 |
