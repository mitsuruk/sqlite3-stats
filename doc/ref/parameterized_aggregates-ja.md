...
> **p 値の有効範囲**: p 値は D の帰無分布に対する Dallal and Wilkinson (1986) の解析的近似から
> 求める。この近似は `p <= 0.10` の範囲で公表されており、`n >= 5` を前提に較正されている。
> 慣用的な有意水準（0.10, 0.05, 0.01）はすべてこの範囲に入るため、棄却の判定は信頼できる。
> 0.10 を超える領域では、標本が正規分布と矛盾しないことを示すだけの値であり、しばしば
> ちょうど `1` が返る。正確な確率として読んだり、2 つの値を比較したりしてはならない。
